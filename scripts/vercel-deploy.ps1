...
function Test-VercelLogin($Token) {
  if ([string]::IsNullOrWhiteSpace($Token)) {
    throw "Vercel CLI is installed, but this script needs non-interactive auth. Run: vercel login and deploy manually, or set VERCEL_TOKEN in .env.local."
  }

  $output = & vercel whoami --token $Token 2>&1
  if ($LASTEXITCODE -ne 0) {
    $message = ($output | Out-String).Trim()
    throw "VERCEL_TOKEN is set but Vercel authentication failed. $message"
  }
...
$vercelToken = Get-ConfigValue "VERCEL_TOKEN" $dotEnv
...
Push-Location $root
try {
  Require-Command "vercel" "Install it with: npm install -g vercel"
  ...
  if (-not $SkipEnv) {
    foreach ($name in $requiredEnv) {
      ...
      foreach ($target in $targets) {
        $envArguments = @("env", "add", $name, $target, "--force")
        if (-not [string]::IsNullOrWhiteSpace($vercelToken)) {
          $envArguments += @("--token", $vercelToken)
        }

        $value | vercel @envArguments
      }
    }
  }

  $deployArguments = @("deploy", "-y")
  if ($Production) {
    $deployArguments += "--prod"
  }
  if (-not [string]::IsNullOrWhiteSpace($vercelToken)) {
    $deployArguments += @("--token", $vercelToken)
  }

  & vercel @deployArguments
} finally {
  Pop-Location
}
